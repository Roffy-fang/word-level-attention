...
function layer:__init(opt)
  ...
  local res_rnn = utils.getopt(opt, 'res_rnn', 0)
  ...
  -- create the core lstm network. note +1 for both the START and END tokens
   ...
   self.core = LSTM_Armour.lstm(self.input_encoding_size, self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.att_size, self.num_layers, dropout_l,dropout_t,res_rnn)
...
function layer:sample_beam(input, opt)
  ...
  local img_x =input
  local batch_size, feat_dim = img_x:size(1), img_x:size(2)
  ...
  for k=1,batch_size do
    ...
    for t=2,self.seq_length+2 do
     ...
     if t == 2 then
      ...
      local inputs = {img_xt,xt,self.init_state[1],unpack(state)}
      local out = self.core:forward(inputs)
      logprobs = out[self.num_state+1] -- last element is the output vector
      state = {}
      for i=1,self.num_state do table.insert(state, out[i]) end
...
function layer:updateOutput(input)
  ...
  for t=2,self.seq_length+2 do
    ...
    if not can_skip then
      ...
      self.inputs[t] = {img_x, xt,self.init_state[1],unpack(self.state[t-1])}
      -- forward the network
