...
if string.len(opt.start_from) > 0 then
  ...
  local irOpt = {}
  --irOpt.input_encoding_size = opt.cnn_outsize
  irOpt.input_encoding_size = opt.input_encoding_size
  irOpt.batch_size = opt.batch_size * opt.seq_per_img
  irOpt.rnn_type = opt.rnn_type
  irOpt.rnn_size = opt.rnn_size
  irOpt.num_layers = opt.num_rnn
  irOpt.dropout_l = opt.drop_prob_lm
  protos.ir = nn.Imgrnn(irOpt)
...
local iparams, grad_iparams = protos.ir:getParameters()
local cnn_params, cnn_grad_params = protos.cnn:getParameters()
print('total number of parameters in LM: ', params:nElement())
print('total number of parameters in IR: ', iparams:nElement())
...
local thin_ir = protos.ir:clone()
thin_ir.corel:share(protos.ir.corel, 'weight', 'bias') -- TODO: we are assuming that LM has specific members! figure out clean way to get rid of, not modular.
thin_ir.corer:share(protos.ir.corer, 'weight', 'bias') -- TODO: we are assuming that LM has specific members! figure out clean way to get rid of, not modular.
local thin_cnn = protos.cnn:clone('weight', 'bias')
-- sanitize all modules of gradient storage so that we dont save big checkpoints
net_utils.sanitize_gradients(thin_cnn)
local lm_modules = thin_lm:getModulesList()
for k,v in pairs(lm_modules) do net_utils.sanitize_gradients(v) end
local ir_modules = thin_ir:getModulesList()
for k,v in pairs(ir_modules) do net_utils.sanitize_gradients(v) end

-- create clones and ensure parameter sharing. we have to do this 
-- all the way here at the end because calls such as :cuda() and
-- :getParameters() reshuffle memory around.
protos.lm:createClones()
protos.ir:createClones()
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    local expanded_feats_cl = protos.ir:forward(expanded_feats_c)
    local logprobs = protos.lm:forward{expanded_feats_cl, data.labels}
    local loss = protos.crit:forward(logprobs, data.labels)
    loss_sum = loss_sum + loss
    loss_evals = loss_evals + 1

    -- forward the model to also get generated samples for each image
    local feats_cl = protos.ir:sample(feats_conv)
    local seq = protos.lm:sample(feats_cl, {beam_size=opt.beam_size})
...
local function lossFun()
 ...
 -- print(feats:size())
  ...
  local dexpanded_feats_cl, ddummy = unpack(protos.lm:backward({expanded_feats_cl, data.labels}, dlogprobs))
  local dexpanded_feats_c,dd = unpack(protos.ir:backward(expanded_feats_c,dexpanded_feats_cl))
   -- print(dexpanded_feats)
  ...
  grad_iparams:clamp(-opt.grad_clip, opt.grad_clip)
...
while true do  
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    if best_score == nil or current_score > best_score then
      ...
      if iter > 0 then -- dont save on very first iteration
        ...
        save_protos.ir = thin_ir -- these are shared clones, and point to correct param storage
  ...
  elseif opt.optim == 'adam' then
    ...
    adam(iparams, grad_iparams, learning_rate, opt.optim_alpha, opt.optim_beta, opt.optim_epsilon, iroptim_state)
  else
